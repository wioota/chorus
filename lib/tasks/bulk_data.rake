...
namespace :bulk_data do
  ...
  namespace :fake_data do
    ...
    task :add_instance, [:admin_username, :instance_name] => :environment do |t, args|
      BulkData.create_gpdb_instance args[:admin_username], args[:instance_name]
    end

    task :add_sandboxes, [:admin_username, :instance_name] => :environment do |t, args|
      BulkData.add_sandboxes args[:admin_username], args[:instance_name]
    end
    ...
    task :load_all, [:admin_username, :workspace_count, :instance_name, :workfiles_per_workspace] do |t, args|
      Rake::Task['bulk_data:fake_data:create_workspaces'].invoke(args[:workspace_count])
      Rake::Task['bulk_data:fake_data:add_instance'].invoke(args[:admin_username], args[:instance_name])
      Rake::Task['bulk_data:fake_data:add_sandboxes'].invoke(args[:admin_username], args[:instance_name])
