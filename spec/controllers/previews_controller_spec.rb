...
describe PreviewsController do
  ...
  describe "#create" do
    ...
    context "when create is successful" do
      before do
        ...
        mock(SqlExecutor).preview_dataset(gpdb_table, connection, check_id) { fake_result }
      end
    ...
    context "when there's an error'" do
      before do
        mock(SqlExecutor).preview_dataset(gpdb_table, connection, check_id) { raise GreenplumConnection::QueryError }
      end
  ...
  describe "#destroy" do
    it "cancels the data preview command" do
      mock(SqlExecutor).cancel_query(gpdb_table, connection, check_id)
  ...
  describe "#preview_sql" do
    ...
    it "returns the results of the sql" do
      mock(SqlExecutor).execute_sql(expected_sql, connection, schema, check_id, :limit => row_limit) { GreenplumSqlResult.new }
