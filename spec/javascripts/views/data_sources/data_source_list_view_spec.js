describe("chorus.views.DataSourceList", function() {
    beforeEach(function() {
        ...
        this.view = new chorus.views.DataSourceList({
    ...
    context('when the data sources are fetched', function() {
        ...
        it('renders the data sources in the correct data source div', function() {
            var dataSources = this.view.$(".data_source li.data_source");
        ...
        it('renders the hadoop data sources in the correct data source div', function() {
            var hadoopItems = this.view.$(".hdfs_data_source li.data_source");
        ...
        it('renders the gnip data sources in the correct data source div', function() {
            var gnipItems = this.view.$(".gnip_data_source li.data_source");
        ...
        describe('when a data source is destroyed', function() {
            beforeEach(function() {
                ...
                liToSelect.click();
                this.selectedId = liToSelect.data("dataSourceId");
            });
            ...
            context("when it is currently selected", function() {
                ...
                it("renders only the existing items", function() {
                    ...
                    expect(this.view.$(".data_source li.data_source").length).toBe(this.oldLength - 1);
                });
            });
            ...
            context('when a non-selected data source is destroyed', function() {
                beforeEach(function() {
                    ...
                    var id = nonSelectedLi.data("dataSourceId");
                    this.dataSources.get(id).destroy();
                    this.server.lastDestroy().succeed();
                });

                it('leaves the same data source selected', function() {
                    expect(this.view.$("li.selected").data("dataSourceId")).toBe(this.selectedId);
                });
            });
        });
        ...
        describe("data_source:added event", function() {
            beforeEach(function() {
                this.newDataSource = rspecFixtures.oracleDataSource({id: 31415});
            ...
            it("selects the li with a matching id when fetch completes", function() {
                this.dataSources.add(this.newDataSource);
                this.view.render(); // re-renders when fetch completes

                expect(this.view.$("li[data-data-source-id=31415]")).toHaveClass("selected");
                expect(this.view.$("li.selected").length).toBe(1);
            });
        });
        ...
        describe('clicking on a gpdbdata source', function() {
            beforeEach(function() {
                this.eventSpy = jasmine.createSpy("data_source:selected");
            ...
            it('triggers the data_source:selected event', function() {
                expect(this.eventSpy).toHaveBeenCalled();
                var dataSourcePassed = this.eventSpy.mostRecentCall.args[0];
                expect(dataSourcePassed.get("name")).toBe("GP9");
            });
        ...
        describe('clicking on a hadoopdata source', function() {
            ...
            it("triggers the data_source:selected event", function() {
                expect(this.eventSpy).toHaveBeenCalled();
                var dataSourcePassed = this.eventSpy.mostRecentCall.args[0];
                expect(dataSourcePassed.get("name")).toBe("Hadoop10");
            });
        ...
        describe('clicking on a gnipdata source', function() {
            ...
            it("triggers the data_source:selected event", function() {
                expect(this.eventSpy).toHaveBeenCalled();
                var dataSourcePassed = this.eventSpy.mostRecentCall.args[0];
                expect(dataSourcePassed.get("name")).toBe("Gnip1");
            });
        ...
        context("when a data source has tags", function () {
            beforeEach(function () {
                var dataSource = this.view.collection.at(0);
                dataSource.tags().reset([{name: "tag1"}, {name: "tag2"}]);
                this.view.render();
            });
            ...
            it("links to the tag show page", function () {
